...
%             layer.b1 = ones(1,1,num_channels); 
            layer.a = ones(1,1,num_channels)*0.5;
%             layer.b2 = 0.1+rand(1,1,num_channels)*0.01;
        ...
        function Z = predict(layer,Xi)
            ...
            scale = max(max(max(max(max(X_p),max(abs(X_n))))))+eps;
%             scale = 50;
            X = Xi/scale;
%             X(X>1) = 1; X(X<-1) = -1;
...
%             layer.a = 0.2;
            ...
            Kp = 0.5*(B1./(Xp - Xp.*alpha + alpha + eps)...
                - (B1-B1.*alpha)./(Xp.*alpha-1 + eps));
            Kn = 0.5*(B2./(-Xn + Xn.*alpha + alpha + eps)...
                - (B2-B2.*alpha)./(-Xn.*alpha-1 + eps));
        ...
        function [dLdX, dLda, dLdb1, dLdb2] = backward(layer, Xi, ~, dLdZ, ~)
...
%             layer.a = 0.2;
            ...
            %% Assign variables
            B1  = layer.b1; B2 = layer.b2; alpha1 = layer.a; alpha2 = layer.a;
